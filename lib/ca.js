 ...
 */
Ca.prototype.checkCert = function (label, certFile, privateKeyFile, publicKeyFile, createFunc, cb) {
  var self = this;
  ...
  Q.all([
    ...
    utils.pExists(privateKeyFile),
    utils.pExists(publicKeyFile)
  ])
...
Ca.prototype.createRootCert = function (passphrase, cb) {
  ...
  Q.nfcall(utils.ensurePath, path.dirname(self.rootCertFile))
  .then(function () {
  ...
  .then(function () {
    ...
    return Q.nfcall(fs.writeFile, self.rootPrivateKeyFile, keys_pem.private);
  })
  .then(function () {
...
Ca.prototype.checkRootCert = function (cb) {
  ...
  // TODO: write wrapper for fs.exists to return a promise
  Q.all([
    utils.pExists(self.rootCertFile),
    utils.pExists(self.rootPrivateKeyFile),
    utils.pExists(self.rootPublicKeyFile)
  ])
  .then(function (results) {
    // any results false?
    if (results.filter(function (r) { return !r; }).length > 0) {
      ...
      Q.nfcall(prompt.get, [{
        name: 'yn',
        message: 'CA Root Certificate missing, generate? (y/n)',
        validator: /^(y|n)$/,
        warning: 'enter y or n',
        required: true,
        hidden: true
      ...
      .then(function (ynres) {
        console.log('answer:', ynres.yn);
        if (ynres.yn !== 'y') {
          throw (new Error('CA Root Certificate missing'));
        }

        Q.nfcall(prompt.get, {
          properties: {
            rootpass: {
              description: 'Enter new Root CA Pass Phrase',
              required: true,
              hidden: true
            }
          }
        ...
        .then(function (rcares) {
          ...
          // install cert
          self.createRootCert(rcares.rootpass, cb);
        });
      });
    }
  });
...
Ca.prototype.createIntermediateCert = function (rootpassphrase, passphrase, cb) {
  ...
  .then(function (encRootPem) {
    ...
    var rootKey = pki.decryptRsaPrivateKey(encRootPem, rootpassphrase);
    ...
    Q.nfcall(utils.ensurePath, path.dirname(self.intCertFile))
    .then(function () {
      // create CA Root Cert Pem file
    ...
    .then(function () {
      // create CA Root Private Key Pem file
      return Q.nfcall(fs.writeFile, self.intPrivateKeyFile, keys_pem.private);
    })
    .then(function () {
      // create CA Root Public Key Pem file
...
Ca.prototype.checkIntermediateCert = function () {

  // check if Intermediate cert exists

  // if not, create csl

  // submit csl to Root CA for auto-signing
  //    call:
  //      checkRootCert()
  //      signCsl(csl, signer_cert, done(err, cert))
 ...
 * @param {Function} callback (err, cert, pem)
 */
Ca.prototype.createAgentSignerCert = function (intpassphrase, passphrase, cb) {
  var self = this,
    cfg = {
  ...
  .then(function (encPem) {
    ...
    var rootKey = pki.decryptRsaPrivateKey(encPem, intpassphrase);
    ...
    Q.nfcall(utils.ensurePath, path.dirname(self.agentSignerCertFile))
    .then(function () {
      // create CA Root Cert Pem file
    ...
    .then(function () {
      // create CA Root Private Key Pem file
      return Q.nfcall(fs.writeFile, self.agentSignerPrivateKeyFile, keys_pem.private);
    })
    .then(function () {
      // create CA Root Public Key Pem file
...
Ca.prototype.checkAgentSignerCert = function () {

  // check if master server cert exists

  // if not, create csl

  // submit csl to Intermediate CA for auto-signing
  //    call:
  //      checkIntermediateCert()
  //      signCsl(csl, signer_cert, done(err, cert))
...
Ca.prototype.createMasterApiCert = function (intpassphrase, cb) {
  ...
  .then(function (encPem) {
    ...
    Q.nfcall(utils.ensurePath, path.dirname(self.masterApiCertFile))
    .then(function () {
      // create CA Root Cert Pem file
    ...
    .then(function () {
      // create CA Root Private Key Pem file
      return Q.nfcall(fs.writeFile, self.masterApiPrivateKeyFile, keys_pem.private);
    })
    .then(function () {
      // create CA Root Public Key Pem file
...
Ca.prototype.checkMasterApiCert = function () {
...
Ca.prototype.createMasterUiCert = function (intpassphrase, cb) {
  ...
  .then(function (encPem) {
    ...
    Q.nfcall(utils.ensurePath, path.dirname(self.masterUiCertFile))
    .then(function () {
      // create CA Root Cert Pem file
    ...
    .then(function () {
      // create CA Root Private Key Pem file
      return Q.nfcall(fs.writeFile, self.masterUiPrivateKeyFile, keys_pem.private);
    })
    .then(function () {
      // create CA Root Public Key Pem file
...
Ca.prototype.checkMasterUiCert = function () {
