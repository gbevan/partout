...
var Ca = function () {
  ...
  self.PARTOUT_AGENT_MANIFEST_DIR = './etc/manifest';
...
Ca.prototype.generateTrustedCertChain = function (cb) {
  ...
  .then(function (certs) {
    ...
    .then(function () {
      self.copyCaCertsToAgentManifest();
      process.nextTick(function () { cb(certs); });
    });
  });

};

Ca.prototype.copyCaCertsToAgentManifest = function (cb) {
  var self = this;
  ...
  .then(function (cert) {
    return Q.nfcall(
      fs.writeFile,
      path.join(self.PARTOUT_AGENT_MANIFEST_DIR, 'root_ca.crt'),
      cert
    );
  })
  .then(function () {
  ...
  .then(function (cert) {
    return Q.nfcall(
      fs.writeFile,
      path.join(self.PARTOUT_AGENT_MANIFEST_DIR, 'intermediate_ca.crt'),
      cert
    );
  })
  .then(function () {
