...
Ca.prototype.copyCaCertsToAgentSslPub = function (cb) {
  ...
  Q.nfcall(utils.ensurePath, self.PARTOUT_SSL_PUBLIC)
  .then(function () {
  ...
  .then(function () {
    return Q.nfcall(utils.ensurePath, self.PARTOUT_AGENT_SSL_PUBLIC);
  })
  .then(function () {
    return Q.nfcall(fs.readFile, self.intCertFile);
  })
...
Ca.prototype.signCsrWithAgentSigner = function (csrPem) {
  var self = this,
  ...
  .then(function (agentPrivatePem) {
    var agentPrivateKey = pki.privateKeyFromPem(agentPrivatePem);
    cert.sign(agentPrivateKey);

    var certPem = pki.certificateToPem(cert);
    //console.log('cert from csr:', certPem);

    deferred.resolve(certPem);
  }).done();
