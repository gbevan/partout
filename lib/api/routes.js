...
//        controllers.agent.upsert({
        appUi.app.service('agents').create({
          id: doc.id,
          ip: doc.ip,
          env: doc.env,
          certInfo: (cert ? {
...
//          controllers.csr.delete(doc._key);
  ...
  r.get('/file', resolveAgent, function (req, res, next) {
    ...
    .then((agent) => {
      ...
      if (f.match(matchAuthRe)) {
        ...
        var envRe = new RegExp(u.format('^%s/%s\/', cfg.MANIFESTDIR, agent.env));
        if (!f.match(envRe)) {
  ...
  r.post('/facts', resolveAgent, requestClientCertAuthorized, function (req, res, next) {
    ...
    debug('/facts posted for agent:', partout_agent_uuid, ip, req.agent);
...
//        controllers.agent.update(doc)
        appUi.app.service('agents').update(partout_agent_uuid, doc)
        .then(() => {
          res.status(200).send('received');
        })
        .catch((err) => {
