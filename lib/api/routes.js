...
var routesApi = function (r, cfg, app, appUi, controllers, serverMetrics) {
  var self = this;
...
//    console.log('services.agents:', app.services.agents);
    app.service('agents').get(cert.subject.GN, {})
    .then(function (doc) {
//      console.log('resolveAgent() get agent:', doc);
      if (doc) {
        req.agent = doc;
      } else {
        req.agent = undefined;
...
//    controllers.csr.register(uuid, ip, csr, env)
    appUi.app.service('csrs').register(uuid, ip, csr)
    .then(function (doc) {
...
//        controllers.agent.upsert({
        appUi.app.service('agent').create({
          id: doc.id,
          ip: doc.ip,
          env: doc.env,
          certInfo: (cert ? {
...
//          controllers.csr.delete(doc._key);
          appUi.app.service('csr').delete(doc.id);
        })
        .done();
      }
    })
...
//          console.log('relname:', v.relname);
  ...
  r.get('/file', resolveAgent, function (req, res, next) {
    ...
    if (f.match(/^etc\/manifest\//)) {
      ...
      var envRe = new RegExp(u.format('^etc\/manifest\/%s\/', req.agent.env));
      if (!f.match(envRe)) {
  ...
  r.post('/facts', resolveAgent, requestClientCertAuthorized, function (req, res, next) {
    ...
    var partout_agent_uuid = req.agent._key,
        now = new Date(),
        ip = getRemoteIP(req);
