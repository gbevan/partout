...
var routesApi = function (r, cfg, db, controllers) {
  var self = this;
   ...
   * The POST request returns the corrent status, and once signed, also
  ...
  r.post('/agentcsr', function (req, res, next) {
    ...
    controllers.csr.register(uuid, ip, csr)
    .then(function (doc) {
  ...
  r.post('/event', function (req, res, next) {
    ...
    var e = req.body;
    console.warn(req.connection.remoteAddress + (e.hostname ? '|' + e.hostname : '') + (e.agent_uuid ? '|' + e.agent_uuid : '') + '> [' + req.body.object + ':' + req.body.module + '] - ' + req.body.msg.trim());
