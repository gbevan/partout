...
//  console.warn('appUi:', appUi);
  ...
  function resolveAgent (req, res, next) {
    ...
    .catch((err) => {
      err.message = 'resolveAgent() err: ' + err.message;
      err.message += `
trying to resolve agent cert:
  CN=${cert.subject.CN}
  OU=${cert.subject.OU}
  GN=${cert.subject.GN}
Agent address:
  ${req.connection._peername.address}
`;
      console.error(err.message);
      // req.connection._peername
...
//        controllers.agent.update(doc)
        return appUi.app.service('agents')
