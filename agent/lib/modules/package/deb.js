...
var Package = P2M.Module(module.filename, function () {
  ...
  .facts(function (deferred, facts_so_far) {
    ...
    .done(function (packages) {
      facts.installed_deb_packages = packages;
  ...
  .action(function (args) {
    ...
    utils.dlog('Package deb: in action ############################ name:', opts.name, 'ensure:', opts.ensure);

    // get current package status

    self.getStatus(opts.name)
    .done(function (details) {
      utils.dlog('Package deb ' + opts.name + ' current state details:', details);

      if (opts.ensure.match(/^(present|installed|latest)$/)) {

        if (!details) {
          console.info('Installing deb package:', opts.name);

          cmd = u.format('dpkg --install %s', opts.name);
      ...
      // ABSENT / PURGED
        if (details) {
          console.info('Removing deb package:', opts.name);

          cmd = u.format('dpkg --purge %s', opts.name);
          exec(cmd, function (err, stdout, stderr) {
            if (err) {
...
Package.prototype.parse_deb = function (out) {
  var packages = {},
      lines = out.split(/\r?\n/);
...
/**
 * Get deb Package status
 * @param {string} name name of deb package
 */
Package.prototype.getStatus = function (name) {
  ...
  .done(function(cp) {
    ...
    cp.on('close', function (rc) {
      if (rc === 0) {
        packages = self.parse_deb(stdout);
      }
